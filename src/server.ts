...
app.post("/groups", async (req, res) => {
    try {
        const newgroup = await prisma.group.create({
            data: {
                users: {
                    connect: req.body.users.map(
                        (useremail : string) => ({email: useremail})
                    )
                },
                name: req.body.name,
...
app.get("/chats/:email", async (req, res) => {
    try {
        ...
        if (groups.filter(group => group.length > 0).length > 0) 
            res.send(groups)
         else 
...
app.post("/sign-in", async (req, res) => {
    try {
        ...
        if (user && verify(password, user.password)) {
            ...
            res.status(400).send({errors: ["Please make sure you are using the right credentials!"]});
        }
    } catch (error) { // @ts-ignore
        res.status(400).send({
            errors: [error.message]
